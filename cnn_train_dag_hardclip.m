function [net,stats] = cnn_train_dag_hardclip(net, imdb, getBatch, varargin)
...
% Edited by GitChoi.
% CNN_TRAIN_DAG with gradient clipping implemented.
% Find -GitChoi for the edited lines.
...
for epoch=start+1:opts.numEpochs
    ...
    if params.epoch<params.lrstep0
    elseif params.epoch<(params.lrstep0+params.lrstep)
        params.learningRate = params.learningRate*0.1;
    elseif params.epoch<(params.lrstep0+2*params.lrstep)
        params.learningRate = params.learningRate*0.1^2;
    else
        params.learningRate = params.learningRate*0.1^3;
    end
    ...
    if opts.plotStatistics
        ...
        drawnow ;
        print(fignum, modelFigPath, '-dpdf') ;
    end
end
    ...
    switch net.params(p).trainMethod
        ...
        case 'gradient'
            ...
            if thisLR>0 || thisDecay>0
                ...
                parDer = 1/batchSize * parDer;
                
                % -GitCHoi
                if params.gclip>0
                    parDer = max(min(parDer,params.gclip),-params.gclip);
                end

                parDer = vl_taccum(1, parDer, ...
...
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) isa(x,'dagnn.my_Loss_pdist'), {net.layers.block})) ;
for i = 1:numel(sel)
